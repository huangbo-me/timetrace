...
- 统计分析：按近三天、本周、上一周、最近 30 天或自定义日期范围查看累计时长、平均时长、趋势与按地点拆分的数据。不同地点类型使用相应的统计口径。
...
- `ChinaWorkCalendar` 只内置 2026 年节假日；其他年份采用普通周末判断。初次引导的常规工作日和工时目前仅保存，不限制实际记录。
- 今天页累计所有工作活动下的工作地点及无地点手动会话时长（备份合并后保留各活动的数据与关联）；跨日会话整段归属开始日，重叠地点时长不去重。统计的全部地点视角使用停留时长等中性文案，单地点视角按地点类型展示。
