...
- 多地点管理：可添加、编辑和删除地点，设置地点名称、类型与围栏半径；地图支持查看全部地点或回到当前位置。
...
- 活动提醒暂不在本期设置页提供添加或管理入口；底层数据、通知处理及已有活动的完成入口保留。
...
- 今天页仅累计默认工作活动下的工作地点及无地点手动会话时长；跨日会话整段归属开始日，重叠地点时长不去重。统计的全部地点视角仍沿用工作类文案，单地点视角按地点类型展示。
